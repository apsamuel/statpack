...
@resp.activate
def test_get_expanded_homicide_counts_by_state_raw_returns_records(fbi_main):
    resp.add(
        resp.GET,
        url="https://example.test/crime/fbi/cde/shr/state/NY",
        json={"totals": {"Homicide Count": 10}},
        match_querystring=False,
    )

    result = fbi_main.get_expanded_homicide_counts_by_state(
...
@resp.activate
def test_get_expanded_homicide_counts_by_state_dataframe_normalizes_columns(fbi_main):
    resp.add(
        resp.GET,
        url="https://example.test/crime/fbi/cde/shr/state/NY",
        json={"totals": {"Homicide Count": 10}},
...
def test_get_expanded_homicide_counts_by_state_calls_sleep_for_all_states(
    ...
    fbi_main.get_expanded_homicide_counts_by_state(state_abbr=None, raw=True)
...
@resp.activate
def test_get_summarized_by_state_raw_returns_payload_list(fbi_main):
    resp.add(
        resp.GET,
        url="https://example.test/crime/fbi/cde/summarized/state/NY/V",
        json=_SUMMARIZED_EMPTY_PAYLOAD,
        match_querystring=False,
    )

    result = fbi_main.get_summarized_by_state(
        state_abbr="NY",
        offense_code="V",
        start_date="01-2025",
        end_date="02-2025",
...
@resp.activate
def test_get_summarized_by_state_dataframe_contains_expected_rows(fbi_main):
    payload = {
        "offenses": {
        ...
        "tooltips": {
            "Percent of Population Coverage": {
                "Coverage": {"2025-01": 80, "2025-02": 81}
            },
            "leftYAxisHeaders": {
                "yAxisHeaderRates": "Rates",
                "yAxisHeaderActual": "Actual",
            },
        },
        "populations": {
            "population": {"Population": {"2025-01": 1000, "2025-02": 1001}},
            "participated_population": {
                "Participated": {"2025-01": 900, "2025-02": 901}
            },
        },
        "cde_properties": {
            "max_data_date": {"UCR": "2025-02"},
    ...
    result = fbi_main.get_summarized_by_state(
        state_abbr="NY", offense_code="V", start_date="01-2025", end_date="02-2025"
    )
...
@resp.activate
def test_get_nibrs_totals_by_state_raw_returns_records(fbi_main):
    resp.add(
        resp.GET,
        url="https://example.test/crime/fbi/cde/nibrs/state/NY/13A",
        json={"actual": 7},
        match_querystring=False,
    )

    result = fbi_main.get_nibrs_totals_by_state(
        state_abbr="NY",
        nibrs_code="13A",
        start_date="01-2025",
        end_date="02-2025",
...
@resp.activate
def test_get_nibrs_totals_by_state_dataframe_uses_ordered_columns(fbi_main):
    resp.add(
        resp.GET,
        url="https://example.test/crime/fbi/cde/nibrs/state/NY/13A",
        json={"actual": 7},
...
def test_get_reporting_agencies_flattens_only_dict_items(
    fbi_main, mocker, mocked_responses
):
