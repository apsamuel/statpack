"""Wave-2 parametrized tests for all public FBI functions.
...
class TestGetReportingAgencies:
    @resp.activate
    def test_raw_true_returns_list(self, fbi_main, mocker):
        mocker.patch.object(fbi_main, "us_territory_mapping", {"TX": {"name": "Texas"}})
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/agency/byStateAbbr/TX",
            json={"TXORI": [{"ori": "TX001", "agency_name": "Austin PD"}]},
            match_querystring=False,
        )
        result = fbi_main.get_reporting_agencies(raw=True)
    ...
    def test_raw_false_returns_dataframe(self, fbi_main, mocker):
        ...
        result = fbi_main.get_reporting_agencies(raw=False)
    ...
    def test_empty_response_returns_empty_dataframe(self, fbi_main, mocker):
        ...
        result = fbi_main.get_reporting_agencies()
        assert isinstance(result, pd.DataFrame)
        assert result.empty
...
class TestGetArrestTotalsByState:
    ...
    def test_raw_true_returns_list(self, fbi_main):
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/arrest/state/NY/all",
            json=ARREST_TOTALS_PAYLOAD,
            match_querystring=False,
        )
        result = fbi_main.get_arrest_totals_by_state(
            state_abbr="NY", start_date="01-2025", end_date="12-2025", raw=True
        )

        assert isinstance(result, list)
        assert len(result) == 1  # whole payload appended as one item

    @resp.activate
    def test_raw_false_returns_list(self, fbi_main):
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/arrest/state/NY/all",
            json=ARREST_TOTALS_PAYLOAD,
        ...
        result = fbi_main.get_arrest_totals_by_state(
            state_abbr="NY", start_date="01-2025", end_date="12-2025", raw=False
        )

        # raw param is currently ignored; function always returns list
        assert isinstance(result, list)
        assert len(result) == 1
...
def _make_counts_payload(state_name: str) -> dict:
    return {
        ...
        "populations": {
            "population": {state_name: {"2025-01": 10000, "2025-02": 10100}}
        },
    }
...
class TestGetArrestCountsByState:
    ...
    def test_raw_true_returns_list(self, fbi_main):
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/arrest/state/NY/all",
            json=_make_counts_payload("New York"),
            match_querystring=False,
        )
        result = fbi_main.get_arrest_counts_by_state(
            state_abbr="NY", start_date="01-2025", end_date="02-2025", raw=True
        )
    ...
    def test_raw_false_returns_indexed_dataframe(self, fbi_main):
        ...
        result = fbi_main.get_arrest_counts_by_state(
            state_abbr="NY", start_date="01-2025", end_date="02-2025", raw=False
        )
    ...
    def test_all_states_iterates_territory_mapping(self, fbi_main, mocker):
        ...
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/arrest/state/CA/all",
            json=_make_counts_payload("California"),
            match_querystring=False,
        )

        result = fbi_main.get_arrest_counts_by_state(raw=True)
...
class TestGetArrestTotalsByOrigin:
    @resp.activate
    def test_raw_true_returns_list(self, fbi_main):
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/arrest/agency/AL0430200/all",
            json=ORIGIN_TOTALS_PAYLOAD,
            match_querystring=False,
        )
        result = fbi_main.get_arrest_totals_by_origin(
    ...
    def test_empty_payload_returns_empty_dataframe(self, fbi_main):
        ...
        result = fbi_main.get_arrest_totals_by_origin(raw=False)
        assert isinstance(result, pd.DataFrame)
        assert result.empty
    ...
    def test_records_contain_rate_and_total_fields(self, fbi_main):
        ...
        result = fbi_main.get_arrest_totals_by_origin(raw=True)
        for record in result:
...
class TestGetArrestCountsByOrigin:
    @resp.activate
    def test_raw_true_returns_list_with_population(self, fbi_main):
        resp.add(
            resp.GET,
            url="https://example.test/crime/fbi/cde/arrest/agency/AL0430200/all",
            json=ORIGIN_COUNTS_PAYLOAD,
            match_querystring=False,
        )
        result = fbi_main.get_arrest_counts_by_origin(
            origin_code="AL0430200",
            offense_code="all",
            start_date="01-2025",
            end_date="02-2025",
    ...
    def test_missing_population_key_falls_back_to_none(self, fbi_main):
        ...
        result = fbi_main.get_arrest_counts_by_origin(raw=True)
    ...
    def test_http_error_propagates(self, fbi_main):
        ...
        with pytest.raises(Exception):
            fbi_main.get_arrest_counts_by_origin(raw=True)
...
_PARAM_CASES = [
    ...
    (
        ...
        "https://example.test/crime/fbi/cde/arrest/agency/AL0430200/all",
        {
            "rates": {"2025-01": 1.0},
            "actuals": {"2025-01": 10},
            "populations": {"population": {"2025-01": 500}},
        },
    ),
    (
...
@resp.activate
def test_raw_true_always_returns_list(
    fn_name, kwargs, url, http_payload, fbi_main, mocker
):
    ...
    assert isinstance(
        result, list
    ), f"{fn_name} with raw=True should return list, got {type(result)}"


@pytest.mark.parametrize("fn_name,kwargs,url,http_payload", _PARAM_CASES)
@resp.activate
def test_raw_false_always_returns_dataframe(
    fn_name, kwargs, url, http_payload, fbi_main, mocker
):
    mocker.patch.object(fbi_main, "us_territory_mapping", {"NY": {"name": "New York"}})
    resp.add(resp.GET, url=url, json=http_payload, match_querystring=False)
    ...
    assert isinstance(
        result, pd.DataFrame
    ), f"{fn_name} with raw=False should return DataFrame, got {type(result)}"
