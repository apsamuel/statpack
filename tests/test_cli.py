...
class TestOutputFormatter:
    ...
    def test_format_markdown_contains_pipe(self, mocker):
        mocker.patch.object(pd.DataFrame, "to_markdown",
...
class TestFBICommandsDispatch:
    ...
    get_cde_* function and pass through argparse Namespace fields."""
    ...
    def test_agencies_calls_get_cde_reporting_agencies(self, mocker):
        mock_fn = mocker.patch.object(self.m, "get_cde_reporting_agencies", return_value=_simple_df())
        result = self.m.FBICommands.agencies(self._args())
        mock_fn.assert_called_once()
        assert isinstance(result, pd.DataFrame)

    def test_arrests_by_state_totals_path(self, mocker):
        mock_fn = mocker.patch.object(self.m, "get_cde_arrest_totals_by_state", return_value=_simple_df())
    ...
    def test_arrests_by_state_breakdown_path(self, mocker):
        mock_fn = mocker.patch.object(self.m, "get_cde_arrest_counts_by_state", return_value=_simple_df())
    ...
    def test_arrests_by_origin_totals_path(self, mocker):
        mock_fn = mocker.patch.object(self.m, "get_cde_arrest_totals_by_origin", return_value=_simple_df())
    ...
    def test_arrests_by_origin_breakdown_path(self, mocker):
        mock_fn = mocker.patch.object(self.m, "get_cde_arrest_counts_by_origin", return_value=_simple_df())
        self.m.FBICommands.arrests_by_origin(self._args(breakdown="by_age"))
        _, kwargs = mock_fn.call_args
        assert kwargs["origin_code"] == "AL0430200"

    def test_nibrs_by_state_returns_dataframe(self, mocker):
        mocker.patch.object(self.m, "get_cde_nibrs_totals_by_state", return_value=_simple_df())
        result = self.m.FBICommands.nibrs_by_state(self._args())
        assert isinstance(result, pd.DataFrame)

    def test_nibrs_by_state_converts_list_to_dataframe(self, mocker):
        mocker.patch.object(self.m, "get_cde_nibrs_totals_by_state", return_value=[{"a": 1}])
        result = self.m.FBICommands.nibrs_by_state(self._args())
        assert isinstance(result, pd.DataFrame)

    def test_expanded_homicide_passes_state(self, mocker):
        mock_fn = mocker.patch.object(self.m, "get_cde_expanded_homicide_counts_by_state",
                                      return_value=_simple_df())
...
class TestCLIIntegration:
    ...
    def test_fbi_agencies_stdout_csv(self, cli_runner, mocker):
        ...
        mocker.patch.object(fbi_pkg, "get_cde_reporting_agencies", return_value=_simple_df())
        stdout, _, code = cli_runner([
            "fbi", "list-reporting-agencies",
            "--output", "stdout",
            "--format", "csv",
        ])
        assert code == 0
    ...
    def test_exception_in_command_exits_with_code_1(self, cli_runner, mocker):
        import pkg.data.sources.fbi as fbi_pkg
        mocker.patch.object(fbi_pkg, "get_cde_reporting_agencies",
